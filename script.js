...
const loginBtn = document.querySelector('.form-container .form-row input[type="login"]');
...
const btnPullLine = document.querySelector('.login-btn-connector');
const btnHandlerCircle = document.querySelector('.login-btn-circle');
...
passwordEl.addEventListener('input', () => { 
    passwordValid = passwordEl.value.length > 5; 
    if (passwordValid) { 
        passwordTl.play(); 
...
function createPullingTimeline(isFixed, BtnPulled) {
    ...
    function animatePullingLine() {
        ...
        const btnWidth = 270;
        const deg = (gsap.getProperty(loginBtn, "rotation") - 4) * Math.PI / 180;
